classdef PostTree < handle
	...
	properties
		...
		ids; % a array of ids in double
		breadths;
	end

	methods
		function pt=PostTree(snopeid)
			pt.snopeid=snopeid;
			pt.root=-1;
			pt.posts={};
			pt.depth=0;
		...
		function update(self)
			...
			while (c<=self.nPosts && ~found)
				if (strcmp(self.posts{c}.status,'a1'))
					found=true;
			...
			if (~found)
				fprintf('root not found/ a1 missing:\n      snopeid: %s\n',self.snopeid);
				return;
			end
